...
import com.hlsfile.bean.other.Email; 

public class SendMail {
	//发件人的邮箱和授权码
	private final String myEmailAccount = "1099397337@qq.com";//发送的邮箱
	private final String myEmailPassword = "bbnsmhfgtaxwfigg";//邮箱秘钥
	...
	public  MimeMessage createMessage(Session session, String sendMail, String receiveMail,String info) throws Exception {
		...
		message.setFrom(new InternetAddress(sendMail, "华莱士简历投递邮箱验证", "UTF-8"));
		// 3. To: 收件人（可以增加多个收件人、抄送、密送）
		message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "xx用户", "UTF-8"));
		// 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
		message.setSubject("华莱士简历投递邮箱验证", "UTF-8");
		// 5. Content: 邮件正文（可以使用html标签）（内容有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改发送内容）
		message.setContent("【邮箱验证码】:"+info+",返回网站输入验证码后,提交信息!", "text/html;charset=UTF-8");
		// 6. 设置发件时间
